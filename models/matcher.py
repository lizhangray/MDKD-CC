...
class HungarianMatcher_Crowd(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        tgt_points = [v["point"] for v in targets]
        # img_size(256, 256) ** 2 / 64 * 4 = 32 * 32 * 4 = 4096

        cost_classes = [
            -output[:, tgt_ids[i]] for i, output in enumerate(out_prob)
        ]
        cost_points = [
            torch.cdist(out_points[i], tgt_points[i], p=2) for i in range(bs)
        ]
        C = [
            self.cost_point * cost_point + self.cost_class * cost_class
            for cost_point, cost_class in zip(cost_points, cost_classes)
        ]

        bs_match = [
            linear_sum_assignment(c.cpu()) for c in C
        ]
        bs_match = [
            (
