...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in data_loader:
        ...
        # print("samples_size in loop:{}".format(samples.size()))
...
def points_crowd_no_overlap(model, data_loader, device, vis_dir=None):
    ...
    # print("transform: {}".format(model.transform))
    # print("evaluate_crowd_no_overlap!")
    save_dir1 = os.path.join(vis_dir,'restoration')
    save_dir2 = os.path.join(vis_dir, 'crowd')
...
def test_crowd_no_overlap(model, data_loader, device, vis_dir=None):
    ...
    from crowd_datasets.Hazy_ShanghaiTechRGBD.loading_data import save_image_tensor
    model.eval()
    # print("here!!!")
    ...
    for samples, targets in data_loader:
        ...
        mae = abs(predict_cnt - gt_cnt)
        mse = (predict_cnt - gt_cnt) * (predict_cnt - gt_cnt)
        print("predict_cnt:{}, gt_cnt:{}".format(predict_cnt, gt_cnt))
...
# the inference routine
@torch.no_grad()
def test_crowd_nAP(model, data_loader, device, vis_dir=None):
    from crowd_datasets.SHHAD.loading_data import save_image_tensor
    # from calculate_nAP import calculate_nAP
    from calculate_nAP_beta import calculate_nAP
    model.eval()
    ...
    for samples, targets in data_loader:
        ...
        points = outputs_points[outputs_scores > threshold].detach().cpu().numpy().tolist()
        predict_cnt = int((outputs_scores > threshold).sum())
        conf = outputs_scores[outputs_scores > threshold].detach().cpu().numpy().tolist()

        # print("len(points):{}, len(conf):{}".format(len(points),len(conf)))
        # if specified, save the visualized images
        ...
        # print("targets: {}".format(targets[0]['point'].detach().cpu().numpy().tolist()))


        nAP = calculate_nAP([[points[i], conf[i]] for i in range(len(points))], targets[0]['point'].detach().cpu().numpy().tolist(), k=3+1, epsilon=0.25)

        print("predict_cnt:{}, gt_cnt:{}, nAP:{}".format(predict_cnt, gt_cnt, nAP))


        maes.append(float(mae))
        mses.append(float(mse))
        nAPs.append(float(nAP))
        show_cnt += 1
    # calc MAE, RMSE
    mae = np.mean(maes)
    mse = np.sqrt(np.mean(mses))
    ...
    return mae, nAP
# mae: 53.02197802197802 nAP: 0.13224329771057927
# mae: 53.02197802197802 nAP: 0.3634300968310375
# mae: 53.02197802197802 nAP: 0.5171566230893306
